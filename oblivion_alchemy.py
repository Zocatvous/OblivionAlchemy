...
class AlchemyFactory:
	...
	def find_common_effects_between_plants(self,df):
		...
		# common_effects = [x.strip().replace('\xa0', '') for x in common_effects]
	...
	def get_polarity_from_effects(self, effects):
		...
		# print(f'effects {effects} is {value}')
		return value
	...
	def generate_magnitude(self, common_effects, verbose=False):
		...
		try:
			if positive_effect is False:
				...
				if duration_only:
					if verbose:print('its a POSITIVE effect thats DURATION_ONLY')
					return 1
				elif magnitude_only:
